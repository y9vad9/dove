package com.dove.backend.storage.database.chats.messages
...
class DatabaseMessagesStorage(
    ...
    private val filesInfoStorage: FilesInfoStorage
) : MessagesStorage {
    ...
    override suspend fun readAll(chatId: Long, number: Int, offset: Long): List<Message> = newSuspendedTransaction {
        ...
        val files = filesInfoStorage.readAll(
            messages.filter {
    ...
    private suspend fun ResultRow.toMessage(): Message {
        ...
        val content: MessageContent = when (type) {
            MessageType.TEXT -> MessageContent.PlainText(message)
            MessageType.MEDIA -> MessageContent.Media(filesInfoStorage.read(UUID.fromString(message))!!)
            MessageType.FILE -> MessageContent.File(filesInfoStorage.read(UUID.fromString(message))!!)
        }
        return Message(
